...
///Programi nderton nje kub dhe nje piramide .Duke perdorur tastieren dhe mouse , keto dy objekte rrotullohen ,
//ndryshojne ngjyre  ,zhduken nga ekrani dhe shfaqen perseri duke perdorur gjuhen C++ dhe librarine OPenGL.
...
void pyramid(int p_frame) {
	...
	glVertex3f(-1.3, 0, -1.3);
	glVertex3f(-1.3, 0, 1.3);//POINT1

	glVertex3f(-1.3, 0, 1.3);
	glVertex3f( 1.3, 0, 1.3);//POINT2

	glVertex3f(1.3, 0, 1.3);
	glVertex3f(-1.3, 0, -1.3);//POINT3
...
void Display(void) {
	...
	if (draw_pyramid) {
		...
		glRotatef(pyramidRotateAngle, -1, -1, -1);
